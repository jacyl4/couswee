...
**Reason**: abtop cache is an external tool artifact under `~/.cache` and is not managed by couswee SQLite state. It also cannot provide reliable multi-account attribution.

**Migration**: Use the account-auth API collector for all accounts with valid auth files. Use SQLite account records as the durable fallback when live collection fails.
